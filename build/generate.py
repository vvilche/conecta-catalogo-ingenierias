...
def card_index(c):
    ...
    sede = (c.get("quien_es") or {}).get("sede", "") if isinstance(c.get("quien_es"), dict) else ""
    ...
    <a class="card ficha-link" href="ficha_{esc(slug)}.html">
      <div class="card-head"><h3>{esc(c["empresa"])}</h3><span class="badge {color.split('-')[1] if False else color}">{esc(label)}</span></div>
...
CSS = """
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .dot.c { background:var(--green); } .dot.s { background:var(--accent); } .dot.comp { background:var(--amber); }
...
{''.join(secs)}
<footer>CONECTA Ingeniería · Catálogo de ingenierías · agosto 2026 · Cada dato cita su fuente verificable</footer>
</div></body></html>'''
...
<span><i class="dot s"></i>SOCIO — alianza técnica complementaria</span>
<span><i class="dot comp"></i>COMPETIDOR — solapamiento a vigilar</span>
</div>
<div class="section-title">Empresas ({len(comps)})</div>
...
<footer>CONECTA Ingeniería · Catálogo de ingenierías · agosto 2026</footer>
</div></body></html>'''
